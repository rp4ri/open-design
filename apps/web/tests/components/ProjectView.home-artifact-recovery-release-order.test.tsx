...
async function runHomeArtifactScenario() {
  ...
  await vi.waitFor(() => {
    ...
    // Manual-document attribution is tracked separately by OPEND-2928.
    // This regression only requires both successful Agent outputs to survive.
...
describe('Terminal artifact finalizers with production provider and mocked HTTP', () => {
  it('keeps both Agent outputs when terminal GET and fallback POST overlap', async () => {
    const message = await runHomeArtifactScenario();
    expect(fileContents.get('document.md')).toBe(manualText);
    expect(fileContents.get('agent-notes.md')).toBe(agentNotes);
    expect(fileContents.get('agent-output.html')).toBe(html);
    expect(message.producedFiles?.map((file) => file.name).sort()).toEqual(expect.arrayContaining(['agent-notes.md', 'agent-output.html']));
