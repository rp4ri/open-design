...
import { streamViaDaemon, reattachDaemonRun, fetchChatRunStatus, createGenericDaemonDisconnectError, DAEMON_STREAM_RECONNECT_LIMIT, type DaemonStreamOptions } from '../../src/providers/daemon';
...
vi.mock('../../src/providers/daemon', async () => {
  ...
  return {
    GENERIC_DAEMON_DISCONNECT_CODE: actual.GENERIC_DAEMON_DISCONNECT_CODE,
    GENERIC_DAEMON_DISCONNECT_MESSAGE: actual.GENERIC_DAEMON_DISCONNECT_MESSAGE,
    createGenericDaemonDisconnectError: actual.createGenericDaemonDisconnectError,
    DAEMON_STREAM_RECONNECT_LIMIT: actual.DAEMON_STREAM_RECONNECT_LIMIT,
...
vi.mock('../../src/components/ChatPane', () => ({
  ChatPane: ({ messages, onSend, viewerOnly, reconnect, onManualReconnect }: {
    ...
    <section>
      {/* Preserve the actual Reconnect UI and its production transport-only callback. */}
      {reconnect ? <Reconnect
        attempt={reconnect.attempt} max={reconnect.max}
        exhausted={reconnect.exhausted} reason={reconnect.reason}
...
describe('ProjectView manual ownership through actual provider disconnect order (OPEND-2928)', () => {
  for (const nullStatusBeforeManualRetry of [false, true]) {
    it(`preserves receipts after failed status, exhausted reconnect and generic error${nullStatusBeforeManualRetry ? ', including a null status followed by the real Reconnect button' : ''}`, async () => {
      ...
      const userIds = [...persisted.values()].filter((message) => message.role === 'user').map((message) => message.id);
      ...
      const runningStatus = {
        ...
        status: 'running' as const, createdAt: runCreatedAt, updatedAt: Date.now(),
        exitCode: null, signal: null,
      };
      vi.mocked(fetchChatRunStatus).mockResolvedValue(nullStatusBeforeManualRetry ? null : runningStatus);
      ...
      await act(async () => {
        // Real provider daemon.ts:2223–2228: these callbacks are synchronous
        // and ordered before its exhausted transport promise returns.
        original.onRunStatus?.('failed');
        original.handlers.onReconnect?.({
          attempt: DAEMON_STREAM_RECONNECT_LIMIT, max: DAEMON_STREAM_RECONNECT_LIMIT, phase: 'exhausted',
        });
        await original.handlers.onError(createGenericDaemonDisconnectError());
        settleOriginalStream?.();
      });
      if (nullStatusBeforeManualRetry) {
        await waitFor(() => {
          expect(fetchChatRunStatus).toHaveBeenCalledWith(originalRunId, null);
          expect(screen.getByRole('button', { name: 'Reconnect' })).toBeEnabled();
          expect(reattachDaemonRun).not.toHaveBeenCalled();
        });
        vi.mocked(fetchChatRunStatus).mockResolvedValue(runningStatus);
        // Real Reconnect button invokes the callback supplied by real PV;
        // no private recovery tick/map or ownership callback is touched.
        fireEvent.click(screen.getByRole('button', { name: 'Reconnect' }));
      }
      await waitFor(() => expect(reattachDaemonRun).toHaveBeenCalledTimes(1));
      const resumed = vi.mocked(reattachDaemonRun).mock.calls[0]![0];
      expect(resumed.runId).toBe(originalRunId);
      expect(resumed.projectId).toBe(project.id);
      expect(resumed.conversationId).toBe(`conv-${project.id}`);
      expect(streamViaDaemon).toHaveBeenCalledTimes(1);
      await successfulAgentWrite(resumed, 'agent.html');
      await finishRun(resumed, ['agent.html'], 'reattach');
