...
let memoryExtractionReady: boolean;
...
beforeEach(() => {
  ...
  reverseHttpCompletion = false; memoryExtractionReady = false;
  ...
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === '/api/memory/extractions' && method === 'GET') {
      const record: MemoryExtractionRecord = {
        id: `memory-extraction-${project.id}`, kind: 'llm', startedAt: Date.now(),
        finishedAt: Date.now(), phase: 'success', userMessagePreview: 'Create a fresh artifact.',
        writtenCount: 1, writtenIds: ['feedback_concise'],
      };
      return Response.json({ extractions: memoryExtractionReady ? [record] : [] });
    }
    if (url.pathname === '/api/memory' && method === 'GET') {
      const entry: MemoryEntrySummary = { id: 'feedback_concise', name: 'Keep replies concise',
        description: 'Prefer concise replies.', type: 'feedback', updatedAt: Date.now() };
      return Response.json({ entries: memoryExtractionReady ? [entry] : [] });
    }
...
// This guard uses the production useMemoryWrittenCard polling/append/save path.
// The fixture only supplies the public extraction and memory HTTP responses;
// it never inserts a host message into React state or the simulated message store.
describe('Artifact recovery with a later host-authored memory message', () => {
  it('keeps the original physical run outputs when the host persists its auxiliary card during a pending artifact response', async () => {
    const { oldAssistantId, oldUserId, oldRunId } = await reachPendingArtifactRecovery();
    expect(causalOrder).not.toContain('html-post-response-released');
    memoryExtractionReady = true;
    // The actual hook polls every 3000ms after the turn ends. Advancing its
    // real interval is independent of the still-unreleased artifact HTTP gate.
    await act(async () => { await vi.advanceTimersByTimeAsync(3000); });
    await vi.waitFor(() => expect(messageWrites.some((message) =>
      message.role === 'assistant' && message.id !== oldAssistantId
      && message.content.includes('Keep replies concise')),
    JSON.stringify({ causalOrder, requests: requests.map(({ method, path }) => ({ method, path })) })).toBe(true));
    const hostMessage = messageWrites.find((message) => message.role === 'assistant'
      && message.id !== oldAssistantId && message.content.includes('Keep replies concise'));
    if (!hostMessage) throw new Error('The production memory poll must append and persist its host message');
    expect(hostMessage).toMatchObject({ role: 'assistant', agentId: 'opencode',
      events: [{ kind: 'text', text: hostMessage.content }], createdAt: expect.any(Number) });
    for (const key of ['runId', 'runStatus', 'startedAt', 'endedAt'] as const) {
      expect(hostMessage[key]).toBeUndefined();
    }
    expect(hostMessage.content).toContain('<od-card type="memory-applied">');
    expect(screen.getByTestId('assistant-produced-files').textContent).toContain(hostMessage.id);
    expect(requests.some((request) => request.method === 'GET' && request.path === '/api/memory/extractions')).toBe(true);
    expect(requests.some((request) => request.method === 'GET' && request.path === '/api/memory')).toBe(true);
    expect(causalOrder).not.toContain('html-post-response-released');
    expect(runCount).toBe(1);
    expect(persisted.get(oldAssistantId)?.runId).toBe(oldRunId);
    expect(persisted.get(oldUserId)?.role).toBe('user');

    await releaseOldArtifactResponse();
    await vi.waitFor(() => expect(persisted.get(oldAssistantId)?.producedFiles?.map((file) => file.name),
      JSON.stringify({ causalOrder, oldAssistant: persisted.get(oldAssistantId), hostMessage }))
      .toEqual(expect.arrayContaining(['agent-notes.md', 'agent-output.html'])));
    expect(persisted.get(oldAssistantId)?.runId).toBe(oldRunId);
    expect(persisted.get(hostMessage.id)?.content).toBe(hostMessage.content);
    expect(persisted.get(hostMessage.id)?.producedFiles ?? []).toEqual([]);
    expect(runCount).toBe(1);
