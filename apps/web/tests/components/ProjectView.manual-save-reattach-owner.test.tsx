...
import { streamViaDaemon, reattachDaemonRun, fetchChatRunStatus, type DaemonStreamOptions } from '../../src/providers/daemon';
...
async function finishWithUnrelatedShell(options: DaemonStreamOptions) {
  await successfulAgentWrite(options, 'agent.html');
  await act(async () => {
    options.handlers.onAgentEvent({
      kind: 'tool_use', id: 'unrelated-shell', name: 'Bash', input: { command: 'pwd' },
    });
    options.handlers.onAgentEvent({
      kind: 'tool_result', toolUseId: 'unrelated-shell', content: '/workspace/owner-test', isError: false,
    });
  });
  await finishRun(options, ['agent.html']);
}

describe('ProjectView actual manual-save and reattach ownership (OPEND-2928)', () => {
  for (const saveMode of ['autosave', 'unmount flush'] as const) {
    it(`excludes a manual Markdown ${saveMode} without relying on the optional file-list refresh callback`, async () => {
      if (saveMode === 'unmount flush') {
        files = [...files, projectFile('existing-preview.html')];
      }
      ...
      const writesBefore = fileRequests.filter((request) => request.method === 'POST').length;
      vi.useFakeTimers();
      try {
        if (saveMode === 'unmount flush') {
          await act(async () => {
            fireEvent.change(editor, { target: { value: '# Manual autosave revision' } });
          });
          expect(editor).toHaveValue('# Manual autosave revision');
          expect(screen.getByRole('heading', { name: 'Manual autosave revision' })).toBeTruthy();
        } else {
          fireEvent.change(editor, { target: { value: '# Manual autosave revision' } });
        }
        if (saveMode === 'autosave') {
          ...
          // Design Files retains the Markdown viewer while hiding it. Opening
          // an existing HTML file removes the non-HTML viewer instead.
          fireEvent.click(screen.getByTestId('workspace-add-tab'));
          await act(async () => {
            // Select the file result, not its separate already-open tab row.
            const fileResults = within(screen.getByTestId('tab-launcher-menu'))
              .getAllByTestId('tab-launcher-result')
              .filter((button) => within(button).queryByText('existing-preview.html', { exact: true }));
            expect(fileResults).toHaveLength(1);
            fireEvent.click(fileResults[0]!);
          });
      ...
      await waitFor(() => {
        expect(fileRequests.filter((request) => request.method === 'POST')).toHaveLength(writesBefore + 1);
        expect(fileContents.get('document.md')).toBe('# Manual autosave revision');
      });
      await finishWithUnrelatedShell(options);
      expect(files.some((file) => file.name === 'document.md')).toBe(true);
      await expectAttribution(['agent.html']);
    });
  }

  it('does not create a manual write when the real Markdown editor is merely opened and closed unchanged', async () => {
    const options = await startRun();
    await createPersonalDocument();
    await screen.findByRole('textbox', { name: /markdown editor/i });
    const writesBefore = fileRequests.filter((request) => request.method === 'POST').length;
    vi.useFakeTimers();
    try {
      await act(async () => { await vi.advanceTimersByTimeAsync(700); });
      await act(async () => { fireEvent.click(screen.getByTestId('design-files-tab')); });
    } finally {
      vi.useRealTimers();
    }
    expect(fileRequests.filter((request) => request.method === 'POST')).toHaveLength(writesBefore);
    await finishWithUnrelatedShell(options);
    await expectAttribution(['agent.html']);
  });

  it('excludes a document manually created after a real ProjectView reattach has started', async () => {
    const startedAt = Date.now();
    const message: ChatMessage = {
      id: 'reattached-assistant', role: 'assistant', agentId: 'byok-opencode',
      content: '', createdAt: startedAt, startedAt,
      runId: 'existing-run', runStatus: 'running', preTurnFileNames: ['input.md'],
    };
    persisted.set(message.id, message);
    vi.mocked(fetchChatRunStatus).mockResolvedValue({
      id: 'existing-run', projectId: project.id, conversationId: `conv-${project.id}`,
      assistantMessageId: message.id, agentId: 'byok-opencode',
    ...
    mountProject({ ...config, mode: 'daemon', agentId: 'byok-opencode' });
    await waitFor(() => expect(reattachDaemonRun).toHaveBeenCalledTimes(1));
    expect(streamViaDaemon).not.toHaveBeenCalled();
    const options = vi.mocked(reattachDaemonRun).mock.calls[0]![0];
    await createPersonalDocument();
    await successfulAgentWrite(options, 'agent.html');
    await act(async () => { options.handlers.onDelta('The reattached work is ready.'); });
    await finishRun(options, ['agent.html'], 'reattach');
    expect(files.some((file) => file.name === 'document.md')).toBe(true);
    await expectAttribution(['agent.html']);
  });
});
