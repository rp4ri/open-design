...
async function seed(strategy = true) {
  ...
  await closeHttp();
  closeDatabase();
  return { root, liveRuns: runs, runId: run.id, snapshotId: snapshot.snapshotId, statePath: path.join(root, 'runs', run.id, 'state.json') };
}
...
// Kept independent of the frozen nine-case red file during causal validation.
// Both fixture copies use production writers/readers; no ownership/projection mock.
describe('cold snapshot compatibility does not widen live or explicit ownership', () => {
  it.each(['missing', 'null', 'empty', 'wrong type'] as const)(
    'does not restore a live run with %s snapshot identity', async (shape) => {
      const fixture = await seed();
      const run = fixture.liveRuns.get(fixture.runId);
      if (!run) throw new Error('Original live service lost its run');
      switch (shape) {
        case 'missing': delete run.appliedPluginSnapshotId; break;
        case 'null': run.appliedPluginSnapshotId = null; break;
        case 'empty': run.appliedPluginSnapshotId = ''; break;
        case 'wrong type': run.appliedPluginSnapshotId = { invalid: true }; break;
      }
      const db = openDatabase(fixture.root, { dataDir: fixture.root });
      expect(getStrategyTaskExecutionByRunId(db, fixture.runId)?.taskExecutionId).toBe(TASK);
      // Re-register HTTP around the ORIGINAL service, without cold hydration.
      const status = await readStatus(await serve(db, fixture.liveRuns, fixture.root), fixture.runId);
      expect(status).toMatchObject({ id: fixture.runId, status: 'succeeded', exitCode: 0 });
      expect(status.strategyTask).toBeUndefined();
    },
  );

  it.each(['null', 'empty', 'wrong type', 'missing immutable owner'] as const)(
    'does not restore cold records with %s', async (shape) => {
      const fixture = await seed();
      const state = JSON.parse(fs.readFileSync(fixture.statePath, 'utf8'));
      switch (shape) {
        case 'null': state.appliedPluginSnapshotId = null; break;
        case 'empty': state.appliedPluginSnapshotId = ''; break;
        case 'wrong type': state.appliedPluginSnapshotId = { invalid: true }; break;
        case 'missing immutable owner':
          delete state.appliedPluginSnapshotId;
          delete state.odNextTaskInputSnapshot;
          break;
      }
      ...
      expect(status).toMatchObject({ id: fixture.runId, status: 'succeeded', exitCode: 0 });
      expect(status.strategyTask).toBeUndefined();
      expect(status.appliedPluginSnapshotId).toBeNull();
    },
  );
});
