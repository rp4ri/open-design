...
async function seed(journal: 'new' | 'legacy', linked = true) {
  ...
  if (linked) linkSnapshotToRun(db, snapshot.snapshotId, run.id);
...
// The accepted task has full immutable identity, but the snapshot lacks the
// additional linked-source witness required for legacy clarification admission.
// A preceding read must not manufacture that write permission.
describe('legacy clarification linked witness is independent of GET request order', () => {
  for (const beforeGet of [false, true]) {
    it.each(['/api/runs', '/api/chat'] as const)(`${beforeGet ? 'after GET' : 'without GET'} rejects an unlinked legacy source through %s`, async (route) => {
      const fixture = await seed('legacy', false);
      const db = openDatabase(fixture.directory, { dataDir: fixture.directory });
      expect(getStrategyTaskExecution(db, TASK)).toMatchObject({
        outcome: 'clarification_required', latestRunId: fixture.sourceRunId,
      ...
      if (beforeGet) {
        const read = await fetch(`${httpFixture.base}/api/runs/${fixture.sourceRunId}`);
        expect(read.status).toBe(200);
        await expect(read.json()).resolves.toMatchObject({
          id: fixture.sourceRunId, status: 'succeeded', appliedPluginSnapshotId: fixture.snapshotId,
          strategyTask: {
            taskExecutionId: TASK, strategy: { snapshotId: fixture.snapshotId },
            outcome: 'clarification_required', inputStage: 'request', terminal: false,
          },
        });
      }
      const response = await fetch(`${httpFixture.base}${route}`, {
        ...
        body: JSON.stringify({
          ...
          userMessageId: 'unlinked-answer', assistantMessageId: 'unlinked-assistant',
          clientRequestId: 'unlinked-client', message: 'Desktop workspace', currentPrompt: 'Desktop workspace',
        }),
      });
      const body: unknown = await response.json();
      expect(response.status, JSON.stringify(body)).toBe(409);
      expect(body).toMatchObject({ error: { code: 'STRATEGY_TASK_SOURCE_RUN_INVALID' } });
      expect(httpFixture.executions).toEqual([]);
      expect(runs.list({ conversationId: CONVERSATION }).map((run) => run.id)).toEqual([fixture.sourceRunId]);
      expect(getStrategyTaskExecution(db, TASK)).toMatchObject({
        outcome: 'clarification_required', clarificationCount: 0, latestRunId: fixture.sourceRunId,
      });
      expect(getStrategyTaskExecution(db, TASK)?.runs).toHaveLength(1);
      expect(httpFixture.requests).toEqual([
        ...(beforeGet ? [{ method: 'GET', path: `/api/runs/${fixture.sourceRunId}` }] : []),
        { method: 'POST', path: route },
      ]);
    });
  }

  it.each(['/api/runs', '/api/chat'] as const)(
    'accepts a linked legacy source after GET successfully restores its task projection through %s',
    async (route) => {
      const fixture = await seed('legacy', true);
      const db = openDatabase(fixture.directory, { dataDir: fixture.directory });
      expect(getStrategyTaskExecution(db, TASK)).toMatchObject({
      ...
      // The preceding GET must restore the read projection without changing POST admission.
      const runs = newRuns(fixture.directory);
      coldRuns = runs;
      const httpFixture = await serve(db, runs, fixture.directory);
      const read = await fetch(`${httpFixture.base}/api/runs/${fixture.sourceRunId}`);
      ...
      expect(httpFixture.requests).toEqual([
        { method: 'GET', path: `/api/runs/${fixture.sourceRunId}` },
        { method: 'POST', path: route },
      ]);
